...
func fieldCoder(fd pref.FieldDescriptor, ft reflect.Type) pointerCoderFuncs {
	...
	case fd.Cardinality() == pref.Repeated && fd.IsPacked():
		...
		case pref.StringKind:
			if ft.Kind() == reflect.String {
				return coderStringPackedSlice
			}
			if ft.Kind() == reflect.Slice && ft.Elem().Kind() == reflect.Uint8 {
				return coderBytesPackedSlice
			}
		case pref.BytesKind:
			if ft.Kind() == reflect.String {
				return coderStringPackedSlice
	...
	case fd.Syntax() == pref.Proto3 && fd.OneofType() == nil:
		// Proto3 fields where the zero value should not be encoded.
		// Oneof fields always encode.
		switch fd.Kind() {
		case pref.BoolKind:
			if ft.Kind() == reflect.Bool {
